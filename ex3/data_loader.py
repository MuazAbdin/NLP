...
class SentimentTreeNode(object):
    def __init__(self, text: list, sentiment_val: float, min_token_idx: int, children=[],
                 parent=None):
        self.text = text
...
class Sentence(object):
    ...
    def __repr__(self):
        return " ".join(self.text) + " | " + SENTIMENT_NAMES[self.sentiment_class] + " | " + str(
            self.sentiment_val)
...
class SentimentTreeBank(object):
    ...
    def __init__(self, path="stanfordSentimentTreebank", split_ratios=(0.8, 0.1, 0.1),
                 split_words=True):
        """
                ...
                for node in leaves:
                    ...
                    if len(splitted_final) > 1 and all([len(s) > 0 for s in splitted_final]):
                        leaves = [SentimentTreeNode([s], node.sentiment_val,
                                                    min_token_idx=node.min_token_idx, parent=node)
                                  for
                                  s in splitted_final]
    ...
    def _build_dataset(self, sentences):
        ...
        with open(os.path.join(self._base_path, DICT_PATH), "r", encoding="utf-8") as f:
            ...
            for line in lines:
                ...
                phrases_dictionary[
                    phrase.lower().replace("-lrb-", "(").replace("-rrb-", ")")] = int(phrase_id)
        ...
        for sent, sent_pointers in zip(sentences, tree_pointers):
            try:
                ...
                for i, p in enumerate(sent_pointers):
                    if i < len(sent):
                        node_text = [sent[i]]
                        node = SentimentTreeNode(text=node_text,
